...
function initializeDisplay() {
    ...
    document.getElementById("inkCost").innerHTML = ink.require.nextCost;
...
function buyTape() {
    tape.cost = Math.floor(tape.require.cleanCash * Math.pow(tape.rate, tape.total));
    ...
    tape.nextCost = Math.floor(tape.require.cleanCash * Math.pow(tape.rate, tape.total));
    document.getElementById("tapeCost").innerHTML = tape.nextCost;
}

function buyPrinter() {
    printer.cost = Math.floor(printer.require.cleanCash * Math.pow(printer.rate,printer.total));
    ...
    printer.nextCost = Math.floor(printer.require.cleanCash * Math.pow(printer.rate,printer.total));
    document.getElementById("printerCost").innerHTML = printer.nextCost;
}

function buyInk() {
    ink.cost = Math.floor(ink.require.cleanCash * Math.pow(ink.rate, ink.total));
    ...
    ink.nextCost = Math.floor(ink.require.cleanCash * Math.pow(ink.rate, ink.total));
    document.getElementById("inkCost").innerHTML = ink.nextCost;
}

function buyMarket() {
    market.cost = Math.floor(market.require.cleanCash * Math.pow(market.rate,market.total));
    ...
    market.nextCost = Math.floor(market.require.cleanCash * Math.pow(market.rate,market.total));
    document.getElementById("marketCost").innerHTML = market.nextCost;
}

function buyGuard() {
    guard.cost = Math.floor(guard.require.cleanCash * Math.pow(guard.rate, guard.total));
    ...
    guard.nextCost = Math.floor(guard.require.cleanCash * Math.pow(guard.rate, guard.total));
...
function buySoap() {
    soap.cost = Math.floor(soap.require.cleanCash * Math.pow(soap.rate, soap.total));
    ...
    soap.nextCost = Math.floor(soap.require.cleanCash * Math.pow(soap.rate, soap.total));
    document.getElementById("soapCost").innerHTML = soap.nextCost;
}

function buyMachine() {
    machine.cost = Math.floor(machine.require.cleanCash * Math.pow(machine.rate,machine.total));
    ...
    machine.nextCost = Math.floor(machine.require.cleanCash * Math.pow(machine.rate,machine.total));
    document.getElementById("machineCost").innerHTML = machine.nextCost;
}

function buyDetergent() {
    detergent.cost = Math.floor(detergent.require.cleanCash * Math.pow(detergent.rate, detergent.total));
    ...
    detergent.nextCost = Math.floor(detergent.require.cleanCash * Math.pow(detergent.rate, detergent.total));
    document.getElementById("detergentCost").innerHTML = detergent.nextCost;
}

function buyLaundromat() {
    laundromat.cost = Math.floor(laundromat.require.cleanCash * Math.pow(laundromat.rate,laundromat.total));
    ...
    laundromat.nextCost = Math.floor(laundromat.require.cleanCash * Math.pow(laundromat.rate,laundromat.total));
    document.getElementById("laundromatCost").innerHTML = laundromat.nextCost;
}

function buyAccountant() {
    accountant.cost = Math.floor(accountant.require.cleanCash * Math.pow(accountant.rate, accountant.total));
    ...
    accountant.nextCost = Math.floor(accountant.require.cleanCash * Math.pow(accountant.rate, accountant.total));
    document.getElementById("accountantCost").innerHTML = accountant.nextCost;
}

// Temporarily reduces tax rate to 0% for an increasing price
function bribePolitician() {
    politicalBribe.cost = Math.floor(politicalBribe.require.cleanCash * Math.pow(politicalBribe.rate, politicalBribe.total));
    if (cleanCash.total > politicalBribe.require.cleanCash) {
        ...
        politicalBribe.nextCost = Math.floor(politicalBribe.require.cleanCash * Math.pow(politicalBribe.rate, politicalBribe.total));
        tax.rate = 0;
        updateTaxRate();
            bribeCount = 5;
